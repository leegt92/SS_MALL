											...
											<c:otherwise>
												<table class="table table-list-search">
													<tr>
														...
														<th style="text-align: center;">환불 수량</th>
														<th style="text-align: center;">환불 가격</th>
														<th style="text-align: center;">환불 일자</th>
														<th style="text-align: center;">환불 상태</th>
													</tr>										
													<c:forEach items="${refundInfo}" var="refundInfo">
													<tr>
														<td style="text-align: center;">
															<a href="/ssmall/productDetail?p_number=${refundInfo.p_number}">
																<img src="/ssmall/productimage/${refundInfo.p_image}" alt="IMG" width="100px" height="100px">
															</a>
														</td>
														<td style="text-align: center; vertical-align: middle;">${refundInfo.p_brand} ${refundInfo.p_name}</td>
														<td style="text-align: center; vertical-align: middle;">${refundInfo.r_amount}</td>						
														<td style="text-align: center; vertical-align: middle;"><fmt:formatNumber value="${refundInfo.r_price}" pattern="###,###,###" />원</td>
														<td style="text-align: center; vertical-align: middle;">${refundInfo.r_date}</td>							
														<td style="text-align: center; vertical-align: middle;">${refundInfo.r_status}</td>										
													</tr>
													</c:forEach>													
												</table>
												...
												<nav aria-label="Page navigation example">
													<ul class="pagination m-5 flex-c-m">				
														<c:if test="${pageMaker.prev}"><!--pageMaker.getprev출력, 트루이게되면 링크를걸음 -->
															<li class="page-item"><a class="page-link" aria-label="Previous" href="/ssmall/admin/refundInfo${pageMaker.makeQuery(pageMaker.startPage - 1)}">«</a></li>
															<!--get방식의 key value를 넘김, 함수를 다이렉트로 추출하는 소스(직접호출) -->
														</c:if>
														<!-- var="idx"하게되면 1씩증가  -->								
														<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
															<c:out value="${pageMaker.cri.page == idx?'':''}" />
															<li class="page-item"><a class="page-link" href="/ssmall/admin/refundInfo${pageMaker.makeQuery(idx)}">${idx}</a></li>
														</c:forEach>
								
														<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
																<li class="page-item"><a class="page-link" href="/ssmall/admin/refundInfo${pageMaker.makeQuery(pageMaker.endPage +1) }">»</a></li>
														</c:if>
